...
Book.find_by_name('Pavans Galliards Almains')
    .songs.create(name: 'Tears of the Muses', song_no: 2, parts_no: 5, has_text: false, pdf_path: "data:application/pdf;base64,#{Base64.encode64(File.read('test.pdf'))}")
Book.find_by_name('Pavans Galliards Almains')
    .songs.create(name: 'Pavan', song_no: 3, parts_no: 5, has_text: false, pdf_path: "data:application/pdf;base64,#{Base64.encode64(File.read('test.pdf'))}")
Book.find_by_name('Lachrimae')
    .songs.create(name: 'Lachrimae Antiquae', song_no: 1, parts_no: 5, has_text: false, pdf_path: "data:application/pdf;base64,#{Base64.encode64(File.read('test.pdf'))}")
Book.find_by_name('Lachrimae')
    .songs.create(name: 'Lachrimae Antiquae Novae', song_no:2, parts_no: 5, has_text: false, pdf_path: "data:application/pdf;base64,#{Base64.encode64(File.read('test.pdf'))}")
Book.find_by_name('First Book of Songs')
    .songs.create(name: 'Unquiet Thoughts', song_no: 1, parts_no: 4, has_text: true)
